...
def SubExample(scene, num1, num2):
    ...
    if posans:
        ...
        DisplayText(scene, f"<span color='Turquoise'>The answer is </span> {ans[1:]}", scale=0.7, wait=5, move=(2, -1),
                    font='Cambria Math')
    else:
        ...
        scene.wait(3)
        DisplayText(scene, f"<span color='Turquoise'>The answer is </span> -{10 ** mlen - int(ans)}", scale=0.7, wait=5,
                    move=(2, -1), font='Cambria Math')
...
def NinesExample(scene, num, mplr, wait=3, fade=True):
    ...
    scene.wait(3)
    scene.play(FadeOut(t, t1))
    t = DisplayText(scene, "5. That sum is our answer.", scale=0.7, wait=5, move=(-3, 0), fade=False,
                    font='Cambria Math')
...
class Nikhilam(Scene):
    def construct(self):
        ...
        t2 = ["सर्वान् अङ्कान् नव त:",  "चरमाङ्कं दश त:"]
        ...
        titex = Text("उदाहरणानि", color='Turquoise', font_size=40)
